...
- [x] AC-8  Entering face-selection mode on a workspace that contains a `Box.ByCenterWidthDepthHeight` (or `Prism`, `Solid`, or panel geometry) visually highlights the mesh surface in the viewport — the mesh faces turn green and are hoverable/clickable. Any mesh geometry created in the scene must be presented as selectable faces in face-selection mode. — fixed 2026-06-06: `src/viewer/geo-selector.js` mousemove handler extended with an `isSelectionModeActive()` guard that raycasts ALL candidate meshes and applies blue hover (`0x89b4fa`) to the hit mesh, teal (`0x94e2d5`) to other candidates, green (`0xa6e3a1`) to already-selected items. The normal panel-hover path is unchanged.
- [x] AC-9  Clicking Approve after selecting a box face returns the actual mesh surface object (a `Mesh` or face geometry with spatial data), NOT a plain string. `Output.Watch` downstream of the `faces` port shows a structured geometry value (has `type`, `vertices`, or equivalent fields — not a bare string label). — fixed 2026-06-06 via 3-file chain: (1) `src/viewer/selection-mode.js` `approveSelection()` now passes full scene-item objects to `onApprove` (not string labels); (2) `src/ui/node-renderer.js` `_activateNodeSelection` onApprove builds `{ _type: 'FaceSelection', label, nodeId, varName }` descriptors and stores them in `nd.controlValues._selectedGeo`; (3) `src/nodes/categories/geometry.js` `Select.Faces/Edges/Points` execute reads `_selectedGeo` and returns typed descriptors with `toString()` returning the label. `Output.Watch` downstream shows descriptor objects (not bare strings). Unit tests in tests/geometry-selection.test.js updated to assert structured output.
