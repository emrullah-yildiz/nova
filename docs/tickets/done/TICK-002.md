...
- [x] AC-7  A Playwright E2E spec in `tests/e2e/geometry-selection.spec.js` covers AC-1 through AC-4 and passes with `npm run test:e2e`. — 10 tests pass: geometry-selection.spec.js:31,54,94,187,285,315,356,409,463,518 — all green (30/30 total E2E). Updated 2026-06-07 by fix/tick-002-selection-interaction: added AC-10 counter and empty-area reset tests.
...
- [x] AC-10  The selection counter in the toolbar always shows the exact count of currently-selected items. Selecting one item shows "1 selected"; selecting a second shows "2 selected"; deselecting one shows "1 selected". The counter does not double-count or swing on a single click. Clicking empty viewport area resets to "0 selected". — E2E: geometry-selection.spec.js:463 (counter 0→1→2→1 on click/deselect), geometry-selection.spec.js:518 (empty-area click resets to 0). `clearSelection()` export added to `src/viewer/selection-mode.js`; geo-selector.js click handler calls it when raycast returns no hits and selection mode is active. Branch: fix/tick-002-selection-interaction 2026-06-07.
