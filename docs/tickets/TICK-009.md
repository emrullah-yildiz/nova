...
## Run comments (2026-06-07 — hover debug logging, branch fix/tick-009f-hover-debug-logging)

**Status:** investigating — hover highlight not visible in browser despite passing E2E tests

**PM confirmed working:** face click counter increments correctly; Approve returns to 2D node canvas.

**Still broken:** Hovering over a face in Select.Faces mode does not turn the face blue in the browser.

**Why E2E passes but browser fails:** The Playwright AC-11 spec injects a fake scene item with hand-crafted `triangleToGroup` and `materials` — it does not exercise the real `toSelectionMesh()` / `traverseVisible()` path. The real hover pipeline needs in-browser logging to identify the exact failure point.

**Next step (T09f):** Add `window.__novaHoverDebug` trace logging to `_swapToFaceMeshes`, the candidate-building loop, and `selectionMeshHover`. PM will open DevTools console after clicking "Select Faces" and report which step is 0/null/undefined. Agent: switch, branch: fix/tick-009f-hover-debug-logging.