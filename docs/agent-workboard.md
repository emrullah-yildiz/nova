...
| fix/tick-002-selection-interaction | ui-engineer (switch) | T02e — TICK-002: Fix selection interaction model: hover needsUpdate + material clone, empty-area click resets selection to 0 via clearSelection(), AC-10 counter E2E tests | `src/viewer/geo-selector.js`, `src/viewer/selection-mode.js`, `tests/e2e/geometry-selection.spec.js`, `src/main.js` (clearSelection added to __selectionModeModule) | `src/main.js` | 2026-06-07 | **merged** |
...
| fix/tick-006-screenshot-viewport | ui-engineer (switch) | T06d — TICK-006: (A) Set viewport to 1280×720, call app.fitToView() before each screenshot, crop PNG to node bounding box + 40px padding; (B) audit all 20 slot builders for wrong Input.Number values or disconnected wires — regenerate all 20 PNGs. | `scripts/take-learning-shots.js`, `public/learning/*.png` | — | 2026-06-07 | **queued** |
| fix/tick-007-number-nodes | ui-engineer (switch) | T07e — TICK-007: Add Input.Number value display to mini-canvas node body (number spinner + range slider matching real Nova canvas). Read node.value from exercise definition. Assert in E2E spec. | `src/ui/mini-canvas.js`, `tests/e2e/learning-interactive.spec.js` | — | 2026-06-07 | **queued** |
