...
| fix/tick-009b-orbit-flat | switch (ui-engineer) | T09b-fix — TICK-009: (A) Replace position-delta orbit guard with _isDragging flag in geo-selector.js; (B) Change selection mesh materials from MeshPhongMaterial to MeshBasicMaterial in geometry-lib.js toSelectionMesh() + remove emissive writes in selection-mode.js; (C) Add orbit-no-reset E2E step | `src/viewer/geo-selector.js`, `src/geometry/geometry-lib.js`, `src/viewer/selection-mode.js`, `tests/e2e/geometry-selection.spec.js` | — | 2026-06-07 | queued |
