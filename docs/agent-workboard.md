...
| fix/selection-mode-e2e | ui-engineer (switch) | T02a — TICK-002: Playwright E2E spec for Select.Faces/Edges/Points + bug fixes | `tests/e2e/geometry-selection.spec.js`, `src/viewer/selection-mode.js` (fix-only), `src/viewer/geo-selector.js` (fix-only), `src/nodes/categories/geometry.js` (fix-only), `src/viewer/viewer3d.js` (fix-only) | — | 2026-06-05 | **active** |
