...
| fix/selection-mode-e2e | ui-engineer (switch) | T02a — TICK-002: Playwright E2E spec for Select.Faces/Edges/Points + bug fixes | `tests/e2e/geometry-selection.spec.js`, `src/viewer/selection-mode.js` (fix-only), `src/viewer/geo-selector.js` (fix-only), `src/nodes/categories/geometry.js` (fix-only), `src/viewer/viewer3d.js` (fix-only) | — | 2026-06-05 | **merged** |
...
| feat/pattern-on-surface-geo | geometry-engineer (mouse) | T04a — TICK-004: Kernel UV remapping, output format, Panel.ByPoints node, PanelFrames compat, Vitest + E2E specs | `src/geometry/geo-facade-panels.js`, `src/geometry/geo-voronoi.js`, `src/nodes/categories/patterns.js`, `tests/geometry/geo-facade-panels.test.js`, `tests/e2e/pattern-on-surface.spec.js` | — | 2026-06-05 | **merged** |
