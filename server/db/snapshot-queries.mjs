// Driver-agnostic snapshot read/write for the EnterpriseStore.
//
// Works against any pool exposing pg's interface: `pool.query(text, params)`
// and `pool.connect()` -> client with `query`/`release`. Both node `pg`
// (PostgresPersistence) and `@neondatabase/serverless` (NeonPersistence) satisfy
// this, so the SQL lives here once. No driver import -> worker-safe.

export async function readSnapshotFromPool(pool, schemaVersion = 1) {
  ...
  return {
    ...
    organizations: organizations.map(r => ({
      id: r.id, name: r.name, slug: r.slug, settings: json(r.settings, {}), createdAt: r.created_at
    })),
    users: users.map(r => ({
      id: r.id,
      email: r.email,
      ...
      aiSettingsEncrypted: r.ai_settings_encrypted || '',
    ...
    projects: projects.map(r => ({
      ...
      members: projectMembers.filter(m => m.project_id === r.id).map(m => ({ userId: m.user_id, role: m.role })),
      versions: versions.filter(v => v.project_id === r.id).map(v => ({
        id: v.id, projectId: v.project_id, createdAt: v.created_at, createdBy: v.created_by, message: v.message, graph: json(v.graph, {})
      }))
    })),
    graphRuns: graphRuns.map(r => ({
      id: r.id, projectId: r.project_id, organizationId: r.organization_id, userId: r.user_id,
      versionId: r.version_id || '', status: r.status, durationMs: r.duration_ms, errorSummary: r.error_summary,
      startedAt: r.started_at, completedAt: r.completed_at
    })),
    objectArtifacts: objectArtifacts.map(r => ({
      id: r.id, organizationId: r.organization_id, projectId: r.project_id, userId: r.user_id,
      name: r.name, kind: r.kind, contentType: r.content_type, byteSize: Number(r.byte_size || 0),
      storageKey: r.storage_key, metadata: json(r.metadata, {}), createdAt: r.created_at
    })),
    backgroundJobs: backgroundJobs.map(r => ({
      id: r.id, organizationId: r.organization_id, userId: r.user_id, projectId: r.project_id || '',
      artifactId: r.artifact_id || '', type: r.type, status: r.status, payload: json(r.payload, {}),
      result: json(r.result, {}), errorSummary: r.error_summary, attempts: r.attempts,
      createdAt: r.created_at, updatedAt: r.updated_at, availableAfter: r.available_after, completedAt: r.completed_at
    })),
    connectorSessions: connectorSessions.map(r => ({
      id: r.id, organizationId: r.organization_id, userId: r.user_id, projectId: r.project_id,
      host: r.host, status: r.status, pairingCode: r.pairing_code, connectorVersion: r.connector_version,
      createdAt: r.created_at, expiresAt: r.expires_at, pairedAt: r.paired_at, lastSeenAt: r.last_seen_at
    })),
    aiRequests: aiRequests.map(r => ({
      id: r.id, organizationId: r.organization_id, userId: r.user_id, projectId: r.project_id,
      provider: r.provider, model: r.model, status: r.status, messageCount: r.message_count,
...
export async function writeSnapshotToPool(pool, snapshot) {
  ...
  try {
    ...
    for (const user of snapshot.users || []) {
      await client.query(
        'INSERT INTO users (id, email, display_name, external_subject, password_hash, email_verified, ai_settings_encrypted, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) ON CONFLICT (id) DO UPDATE SET email=EXCLUDED.email, display_name=EXCLUDED.display_name, external_subject=EXCLUDED.external_subject, password_hash=EXCLUDED.password_hash, email_verified=EXCLUDED.email_verified, ai_settings_encrypted=EXCLUDED.ai_settings_encrypted, created_at=EXCLUDED.created_at',
        [user.id, user.email, user.displayName || '', user.externalSubject || '', user.passwordHash || '', user.emailVerified === true, user.aiSettingsEncrypted || '', user.createdAt]
      );
