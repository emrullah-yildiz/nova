...
export function installPortHandler(targetApp = getRuntimeApp(), runtimeGlobal = getRuntimeGlobal()) {
  if (!targetApp) return false;
  if (targetApp.__portHandlerInstalled) return true;
  targetApp.__portHandlerInstalled = true;
  const app = targetApp;
  ...
  function install() {
    ...
    app.onWireEnd = function(e) {
      ...
      if (tgt && tgt.classList.contains('port-dot')) {
        ...
        if (ds.isNew && Math.sqrt(dx*dx + dy*dy) > 20 && typeof runtimeGlobal.showPortSuggest === 'function') {
          var nid = ds.fromNode, pid = ds.fromPort, pDir = ds.fromDir;
          var pType = app._dragPortType || 'any';
          this.connectingWire = null;
          app._dragState = null;
          this.renderWires();
          setTimeout(function() { runtimeGlobal.showPortSuggest(e, nid, pid, pType, pDir); }, 50);
          return;
        }
      }
...
export default installPortHandler;
