...
// Auto-built mapping of Geo.<method> → { type, ports[] } derived from
// each node's codegen.python template. Lets new composites be
// recognised by the parser without editing this file — the registry
// IS the source of truth. Built lazily on first parse so node-
// registry imports have completed.
let _autoGeoMap = null;

function _extractGeoSignatureFromCodegen(template) {
  if (typeof template !== 'string' || !template) return null;
  if (template.indexOf('\n') !== -1) return null;
  const m = template.match(/Geo\.([A-Za-z_][A-Za-z0-9_]*)\s*\(([\s\S]*)\)\s*$/);
  if (!m) return null;
  const method = m[1];
  const argsRaw = m[2];
  const ports = [];
  ...
  for (let i = 0; i < argsRaw.length; i++) {
    ...
    if (ch === ',' && depth === 0) { ports.push(cur); cur = ''; continue; }
    cur += ch;
  }
  if (cur.trim()) ports.push(cur);
  const inputPortIds = ports.map(function (arg) {
    const ph = arg.match(/\{\{([A-Za-z_][A-Za-z0-9_]*)\}\}/);
    return ph ? ph[1] : null;
  });
  return { method: method, inputPortIds: inputPortIds };
}

function _buildAutoGeoMap() {
  const map = {};
  for (const typeId of Object.keys(NODE_TYPE_MAP)) {
    const def = NODE_TYPE_MAP[typeId];
    const codegenPy = def && def.codegen && def.codegen.python;
    const sig = _extractGeoSignatureFromCodegen(codegenPy);
    if (!sig) continue;
    const fullName = 'Geo.' + sig.method;
    if (!map[fullName]) {
      map[fullName] = { type: typeId, inputPortIds: sig.inputPortIds };
    }
  }
  return map;
}

function _getAutoGeoMap() {
  if (!_autoGeoMap) _autoGeoMap = _buildAutoGeoMap();
  return _autoGeoMap;
}

export function _resetAutoGeoMapForTests() {
  _autoGeoMap = null;
}
...
const CodeParser = {
  ...
  analyzeExpr(expr) {
    ...
    if (funcMatch) {
      ...
      // ── REGISTRY-DRIVEN auto-mapping ──
      // Built from every node's codegen.python so newly-added composites
      // (Pattern.TwistedEllipsePlates, Pattern.HexPanelGrid, etc.) are
      // recognised here automatically. Falls through to the hardcoded
      // legacy table below only when the auto-map doesn't have an entry.
      const autoMap = _getAutoGeoMap();
      const autoEntry = autoMap[fn];
      if (autoEntry) {
        const inputs = {};
        for (let i = 0; i < autoEntry.inputPortIds.length; i++) {
          const portId = autoEntry.inputPortIds[i];
          if (!portId) continue;
          inputs[portId] = args[i] !== undefined ? args[i] : '_';
        }
        return { type: autoEntry.type, inputs: inputs };
      }

      // ── GEO LIBRARY → VISUAL NODES (hardcoded fallback) ──
